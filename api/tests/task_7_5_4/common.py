...
def test(output_lines, test_number, correct_lines, vertex_count, color_count, edges):
    ...
    if len(nums) != vertex_count or len(set(nums)) != color_count:
        return (test_number, "WA: output mismatch", output_lines, correct_lines)

    for (f, t) in edges:
        if f != t and nums[f - 1] == nums[t - 1]:
            return (test_number, "WA: output mismatch", output_lines, correct_lines)
